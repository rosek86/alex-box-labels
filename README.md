...
### Future PART-DB integration

PART-DB integration is not implemented yet. An adapter can map selected parts into the existing project format without changing layout or printing. JSON generated by an external JavaScript script can already be imported.

A future adapter storing a PART-DB token must run on a separate backend when the interface is hosted on GitHub Pages. Keep credentials out of static assets, exported projects and `localStorage`. Choose endpoints, authentication, pagination and field mapping against the documentation for the actual PART-DB instance.
